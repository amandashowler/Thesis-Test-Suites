...
class TestParser:
    def __init__(self, UserDB, AvailableGameDB, GameCollectionDB, config):
        self.userDB = UserDB
        self.available_gameDB = AvailableGameDB
        self.game_collectionDB = GameCollectionDB
        self.config = config
        self.curr_line = 0
        self.prev_param = ""
        ...
        self.logout_name = config['logout']['transaction_name']
    ...
    def start_parse_get_user_and_logout(self, test_lines):
        ...
        # created self.logout_name in case logout isn't their transaction
        try:
            logout_line = test_lines.index(self.logout_name)
        except ValueError:
            logout_line = -1
    ...
    def check_transaction_list(self, transaction, input):
        transaction_list = ["addcredit", "sell", "buy", "create", "delete", "refund"]
