...
def main():
    ...
    for filename in os.listdir("config_files"):
        ...
        game_collection_fp = f"project_files/{course_project_title}/databases/game_collection_file.txt"
        game_fp = f"project_files/{course_project_title}/databases/available_games_file.txt"
        test_input_file_endswith = config['file']['test_input_file_endswith']
        
        # create system and process test
        system = BucketSystem(course_project_title, test_input_file_endswith, test_input_fp, user_fp, game_collection_fp, game_fp, config)
