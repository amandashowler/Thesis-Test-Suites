...
class BidParser(TestParser):
    ...
    def helper_bid_invalid_user_type(self, test_lines, user_type):
        ...
        if (user_type == "ss"):
            if (len(test_lines) <= self.logout_other_transaction) or (test_lines[self.logout_other_transaction] == "logout"):
    ...
    def validate_new_bid(self, bid):
        ...
        # check if bid is not 5% higher, it fails
